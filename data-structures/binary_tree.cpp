...
public:
  ...
  bool deleteNode(const T &element) {
    ...
    std::unique_ptr<Tree<T>> target_left_child =
        target_node->child_left_ ? std::move(target_node->child_left_)
                                 : nullptr;
    std::unique_ptr<Tree<T>> target_right_child =
        target_node->child_left_ ? std::move(target_node->child_left_)
                                 : nullptr;
    ...
    if (target_left_child && target_right_child) {
      ...
      std::unique_ptr<Tree<T>> predecessor = std::move(target_left_child);
      ...
      predecessor->child_left_ = std::move(target_left_child);
      predecessor->child_right_ = std::move(target_right_child);
      ...
      predecessor_root->child_right_.reset();
    ...
    if (target_left_child) {
      if (is_left) {
        target_root->child_left_ = std::move(target_left_child);
      } else {
        target_root->child_right_ = std::move(target_left_child);
      }
    } else {
      if (is_left) {
        target_root->child_left_ = std::move(target_right_child);
      } else {
        target_root->child_right_ = std::move(target_right_child);
      }
    }
